 ...
 * ------------------------------------------------------------------
 * test_performance.c — Parser throughput sweep (all sample files)
 * ------------------------------------------------------------------
 ...
 *   test/benchmarks/test_bench_parse.c      — focused parse stats
 *   test/benchmarks/test_bench_resolver.c   — resolver cold/warm
 *   test/benchmarks/test_bench_serializer.c — writer throughput
 *   test/benchmarks/test_bench_asl.c        — ASL engine throughput
 * ------------------------------------------------------------------
 */
#include "anvil.h"
...
static void set_config(FILE **logger) {
   *logger = fopen("logs/test_performance.log", "w");
   printf("\n  %-28s  %8s  %10s  %6s  %10s\n",
