...
/* PS14 — anonymous object block with inheritance: Child : Parent { ... } */
static void test_ps14_anon_block_inheritance(void) {
    const char *source =
        "Parent := { hp := 100 }\n"
        "Child : Parent { hp := 125 }";
    ...
    TestBit.is_not_null(ctx, "PS14: context created");

    bool ok = Context.parse(ctx);
    TestBit.is_true(ok, "PS14: source parsing succeeds");
    if (!ok) {
        Context.dispose(ctx);
        return;
    }
    ...
    TestBit.is_true(Context.statement_count(ctx) >= 2,
                    "PS14: at least 2 statements");

    statement stmt = Context.get_statement(ctx, 1);
    TestBit.is_not_null(stmt, "PS14: stmt[1] exists");
    TestBit.is_equal_int(ANVL_ANON_OBJECT, (long long)Statement.type(stmt),
                         "PS14: stmt[1] is anonymous object block");
    TestBit.is_not_null(stmt->base_meta,
                        "PS14: stmt[1] has base_meta for inheritance");

    Context.dispose(ctx);
}

/* PS15 — anonymous object block inheritance with attributes */
static void test_ps15_anon_block_inheritance_with_attributes(void) {
    const char *source =
        "Base := { value := 1 }\n"
        "Derived : Base @[tier=elite] { value := 2 }";

    ctx_builder builder = Context.get_builder();
    builder->set_source(builder, source, strlen(source));
    builder->set_dialect(builder, ANVL_DIALECT_AML);
    context ctx = builder->build(builder);
    TestBit.is_not_null(ctx, "PS15: context created");

    bool ok = Context.parse(ctx);
    TestBit.is_true(ok, "PS15: source parsing succeeds");
    if (!ok) {
        Context.dispose(ctx);
        return;
    }
    ...
    TestBit.is_true(Context.statement_count(ctx) >= 2,
                    "PS15: at least 2 statements");

    statement stmt = Context.get_statement(ctx, 1);
    TestBit.is_not_null(stmt, "PS15: stmt[1] exists");
    TestBit.is_equal_int(ANVL_ANON_OBJECT, (long long)Statement.type(stmt),
                         "PS15: stmt[1] is anonymous object block");
    TestBit.is_not_null(stmt->base_meta,
                        "PS15: stmt[1] has base_meta for inheritance");
    TestBit.is_true(stmt->meta[STMT_META_ATTR_IDX] >= 1,
                    "PS15: stmt[1] retains attribute metadata");
...
int main(void) {
    ...
    TestBit.run_ex("PS14_anon_block_inheritance",      NULL, test_ps14_anon_block_inheritance,      td);
    TestBit.run_ex("PS15_anon_block_inheritance_with_attributes", NULL, test_ps15_anon_block_inheritance_with_attributes, td);
