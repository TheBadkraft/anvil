...
- **Resolved and implemented — `tuple` is AMP-legal.** Supersedes the "still undecided" note this line originally carried. Decided (unanimous, cross-project with `anvil.js`, which already ships this): a scalar-only tuple is a real, practical AMP use case, same footing as a scalar array. `parse_tuple` (`src/core/parser.c`) mirrors `parse_array`'s shape — reject-on-sight of a nested collection element for AMP, min two elements (empty/single-element tuples are both errors, reusing `ANVL_ERR_PARSER_EMPTY_TUPLE_NOT_ALLOWED`/`ANVL_ERR_PARSER_TUPLE_TOO_FEW_ELEMENTS`, and `ANVL_ERR_AMP_ARRAY_ELEMENT_NOT_SCALAR` for the AMP restriction — matching the legacy parser's own choice to reuse the array error code rather than a separate tuple one). `AMP17` is written as the positive case (tuple-of-scalars accepted) — see **AMP suite** below. Element parsing is scalar-only for now (`parse_scalar_value`, same as array) — AML's eventual "a tuple element may be any value, including a nested array/object" is deferred until object/array-value dispatch exists.
...
**Status: both suites complete and GREEN.** `test/unit/test_body_amp.c` (29 cases, `AMP00a`–`AMP23`, including `AMP17`) — all 29 passing (203/203 assertions). `test/unit/test_body_aml.c` (13 cases, `AML00`–`AML12`) — all 13 passing (91/91 assertions). Both Valgrind-clean (0 errors, 0 leaks — confirmed after the `.collection.items`/`.attributes`/`.body` `mod_ctx_dispose` fixes, see **Arena-backed allocation** — not before). The `#if ENABLED`/`#endif` scaffolding that gated cases in while `doc_parse_body` was still a stub has been removed entirely now that nothing needs gating — every case in both files compiles and runs unconditionally.
...
### AMP suite — `test/unit/test_body_amp.c`, cases AMP00a–AMP23
...
- **AML00** — tuple assignment, `f06_tuple.anvl` (`coords := (10, 20);`) — currently just a plain scalar tuple, same shape `AMP17` already covers; the AML-specific case this was meant to add — a tuple containing a non-scalar element (nested object/array/tuple) — still needs a fixture/assertion update once tuple elements accept any value, not just scalars (deferred, noted on `parse_tuple` itself).
