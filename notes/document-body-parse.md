...
Implementation reference for the body-parse step that follows header scanning and include loading in AML.
...
1. **Header scan** — discover shebang, includes, and module attributes.
2. **Include loading** — expand the include graph recursively.
3. **Body parse** — parse statements and build a document-level statement/value tree.
4. **Resolution** — resolve inheritance, references, and includes after all bodies are parsed.
...
- Enforce dialect-legal grammar inline, failing fast via `Source.set_error` the moment a construct isn't legal for the source's dialect — the same pattern the header scanner already uses for `ANVL_ERR_INCLUDE_AMP_FORBIDDEN` and AMP attribute rejection. See **Dialect scope** below.
...
AMP is a flat, minimal message-packet format, not a module — its header can never carry includes or attributes (`header_scan_includes`/`header_scan_attributes` already enforce this), so its body grammar is correspondingly narrow:
...
- Input: a document whose header has been scanned and whose includes have been loaded.
...
This function is exposed for independent testing and is called internally after `mod_load_includes` succeeds.
...
See `anvil.js/anvl-js-parser-spec.md` §5.6 for the full authoritative design (this project's own decision now matches it). **Implemented** (`body-parse-nested-varref` branch): `parse_varref` in `src/core/parser.c`, dispatched from `parse_value_body` on a leading `$` (before `parse_bare_literal` is ever tried, since `$` was already excluded as a valid bare-literal leading character). `ANVL_VALUE_VARREF` carries the target identifier in `.varref.target` (sigil excluded) and the full `$identifier` span in `.text`. Malformed `$` (missing target, whitespace before it, or a dotted-path target like `$foo.bar`) all reuse `ANVL_ERR_VARS_INVALID_VARREF` (5010); AMP rejects `$` entirely, at any depth, reusing `ANVL_ERR_PARSER_UNEXPECTED_TOKEN` (3100), the same code every other AMP-forbidden construct uses. The VarRef target's character scan does **not** apply `parse_identifier`'s keyword rejection — a keyword-shaped target (`$include`) is syntactically valid and simply resolves to `null` at Resolution time, consistent with target existence never being checked at parse time. See `test_body_aml.c` (AML02, AML14–AML16) and `test_body_amp.c` (AMP25).
...
**Found and fixed much later, via `public-api.md`'s accessor work — `parse_string_literal`/`parse_blob_literal` both captured `.text.end` one byte too late.** Both functions checked for the closing delimiter (`"`/`` ` ``), then called `Source.consume(src, 1)` to consume it, and only *after* that assigned `.text.end = Source.at(src)` — by which point the cursor had already moved past the closing delimiter, so `.text` silently included it as a trailing byte (`"hello"`'s `.text` was actually `hello"`, six bytes not five). No existing body-parse test caught this — none of them checked a scalar's exact `.text` length or last byte, only its content via a prefix-tolerant `slice_equals`-style comparison. Anvil Native's `anvil_value_get_text` (`test_anvil_native.c`'s `ANV10`/`ANV11`) was the first thing to actually assert an exact resolved length, and caught it immediately. Fixed in both functions by capturing `end` into a local *before* consuming the closing delimiter, then assigning `.text.end = end` afterward — same one-line shape in both places.
...
- **Resolved — arena scope is per-module, shared across every document the module owns.** Not per-`module_document` — one arena for the whole include graph, so every document's statements and values end up contiguous in the same block. Corrected from an earlier draft of this note that read "module-based" as per-document; the actual intent is the reverse — one arena, all documents, deliberately for locality when navigating the combined tree.

  **Ownership hierarchy, clarified**: `AnvlMod` (the module) is the owning container; `module_context` is "an inner organizer" beneath it (owns the `docs`/`errors`/`parser` bookkeeping — the practical aggregate of the whole include graph); `module_document` belongs to both, one per source file. A document is named `module_document` because it belongs to a module, not because it *is* one — the ambiguity that prompted this correction. Worth revisiting the naming at some point; not urgent.

  **Sizing implication**: capacity should be the *sum* of `Source.length()` across every document in `ctx->docs`, not one document's length alone. This is computable up front — by the time `doc_parse_body` runs for any document, the whole include graph is already loaded (`mod_load_includes` has fully expanded it), so the complete document set — and the complete sum — is known before body-parsing starts for the first one.

  **Resolved — coupled lifetime is intentional, not a limitation.** A document's parsed body was never independently useful once something it references (an include, a `base`) is gone — the coupling just makes an already-true dependency explicit rather than imposing a new one. `module_context` owns the arena (not the individual document): `module_document` already carries a `context` back-pointer (`doc->context`), and `mod_ctx_dispose` already tears down `ctx->docs` as one atomic unit — adding the arena there means teardown gets *simpler* than today, not more complex. Today `doc_dispose` walks `doc->body` disposing each statement individually; once the arena lands, `mod_ctx_dispose` just frees the one arena and every document's parsed output goes with it in one shot, which is the actual point of going arena-backed in the first place.

  **Sequencing**: the arena can't be sized (and shouldn't be allocated) until the full document set is known — which is exactly true right after `mod_load_includes` finishes expanding the graph. "Size and allocate `ctx`'s arena" is one new step inserted between include-loading and body-parsing, not something each document coordinates individually. All pieces below are implemented and tested individually (and chained together by hand in `CR21`/`CR22`) — what's missing is this exact sequence appearing in real (non-test) code:
  ```
  mod_load_includes(ctx, root, &size_hint, &err)      // full doc graph now known
  ...
  **Open**: does "one shot" disposal require the `list` backing storage itself (Sigma `list`'s dynamic array behind `doc->body`, `value->collection.items`, `value->object.statements`, `statement->attributes`) to also be carved from the arena, or do those stay on the regular heap via `List.new`? If lists stay on the heap, bulk-freeing the arena reclaims every *node*, but the (much smaller number of) *list* structures still need individually disposing — fewer objects to walk than today, but not literally zero. Worth deciding when the arena's actual allocation API takes shape; not blocking the design above.
  ...
  Header-scan data (`doc->header`'s `anvl_include`/`anvl_doc_attribute` lists) stays **outside** the arena regardless of scope, on its existing `Allocator.alloc`/`List.new` path — the arena is specifically for `doc_parse_body`'s output (statements and values), not header metadata.
...
**Status: both suites complete and GREEN.** `test/unit/test_body_amp.c` (31 cases, `AMP00a`–`AMP25`, including `AMP17`) — all 31 passing (219/219 assertions). `test/unit/test_body_aml.c` (17 cases, `AML00`–`AML16`) — all 17 passing (139/139 assertions). Both Valgrind-clean (0 errors, 0 leaks — confirmed after the `.collection.items`/`.attributes`/`.body` `mod_ctx_dispose` fixes, see **Arena-backed allocation** — not before). The `#if ENABLED`/`#endif` scaffolding that gated cases in while `doc_parse_body` was still a stub has been removed entirely now that nothing needs gating — every case in both files compiles and runs unconditionally.
...
### AMP suite — `test/unit/test_body_amp.c`, cases AMP00a–AMP25
...
- **AMP00b** — `include` rejected as an ordinary statement identifier, expecting `ANVL_ERR_PARSER_IDENTIFIER_IS_KEYWORD`. Deliberately *not* the first statement in the buffer — see the gotcha below.
- **AMP00c** — empty body returns OK and an empty statement list.

**Gotcha, worth knowing before writing another keyword-rejection fixture**: `include` as the *first* statement in an AMP document doesn't reach the body parser at all. `header_scan_includes` (`src/core/document.c`) word-boundary-matches a leading `include` token and, since AMP forbids includes outright, fails immediately with `ANVL_ERR_INCLUDE_AMP_FORBIDDEN` at header-scan time — `setup_amp_doc` (`test/utilities/helpers.c`) returns early on that failure, before `mod_load_includes`/`mod_ctx_create_arena` even run, so `doc_parse_body` never gets a properly-set-up document to work with. `header_scan_includes`'s include-loop only scans *leading* includes and stops for good the moment it sees non-include content, so putting a real statement first sidesteps this — `include` as a later statement genuinely reaches body-parse untouched. First version of `AMP00b` didn't do this and accidentally "passed" its `doc_has_errors` check for the wrong reason (a header-scan error, not the intended identifier-keyword rejection) — caught by asserting the *specific* error code, not just "an error happened."
...
- **AMP21** — a RESERVED keyword (`include`/`vars`/`using`) rejected as a bare *value*, not just as an identifier — the other half of the invariant `AMP00a`/`AMP00b` establish for identifier position, `ANVL_ERR_PARSER_VALUE_IS_KEYWORD`.
...
- **AMP25** — `$identifier` VarRef rejected entirely, both at the top-level value position and nested as an array element — `ANVL_ERR_PARSER_UNEXPECTED_TOKEN`, the same code every other AMP-forbidden construct uses; falls out naturally from `parse_value_body`'s own AMP gate, no special-casing needed in `parse_collection`'s existing nested-collection pre-check.

### AML suite — `test/unit/test_body_aml.c`, cases AML00–AML16

**Status: complete and GREEN.** 17/17 passing, 139/139 assertions, Valgrind-clean. `AML01`/`AML03`/`AML05`–`AML08` (everything needing object/`OBJECT_BLOCK`, attributes, or inheritance) were the last to land, all together in one pass — `parse_statement_list` (`src/core/parser.c`) is the shared recursive helper both forms use: `parse_object_value` reaches it through `:=` (producing `ANVL_VALUE_OBJECT`), `parse_statement` reaches it directly for the bare `ident { ... }` form (`ANVL_STMT_OBJECT_BLOCK`). Every nested statement inside a `{ }` body is created via the same `Source.new_node`/`parse_statement` machinery as a top-level one, so it's automatically indexed in `ctx->statements`/`ctx->values` at any nesting depth — `mod_ctx_dispose`'s existing flat disposal passes (see **Arena-backed allocation**) needed no recursion added, just two more list-kinds to release per statement (`.body`, alongside `.attributes`).
...
- **AML02** — static value reference (`f11_static_ref.anvl`, `derived_var := $base_var;`) — `VARREF` value node, target `base_var`; updated from an earlier `IDENTIFIER`-based version once `$` VarRef reinstatement actually landed (see **Fixture coverage** below).
...
- **AML11** — include + static reference into the flat merged namespace (`f13_include.anvl`).
...
- **AML14** — `$identifier` VarRef, `body_varref.anvl` (new fixture) — as a statement's own value (`alias := $name;`) and nested inside an array element (`wrapped := [$name, "static"];`), proving VarRef flows through the same `parse_value_body` dispatch every other value kind uses, at any depth, with no special-casing needed in `parse_collection`.
- **AML15** — malformed `$` with no target following it — nothing at all, or whitespace between `$` and the identifier — reports `ANVL_ERR_VARS_INVALID_VARREF` (inline buffers, not a fixture — too small to be worth one).
- **AML16** — dotted-path `$name.sub` rejected — `$identifier` is strictly bare, matching `anvil.js`'s `parseVarRef` precedent of naming this specific mistake rather than silently truncating to `$name` and letting the leftover `.sub` trip a confusing, unrelated failure — reports `ANVL_ERR_VARS_INVALID_VARREF`.
...
- **Stale fixtures cleaned up**:
  - `f11_varref.anvl` (used `$host` var-refs) → replaced with `f11_static_ref.anvl`, originally `base_var := 42; derived_var := base_var;` (a bare-identifier reference, matching the design position that was current at the time). That position was later superseded by the `$` VarRef reinstatement (see **Value structure**'s "`$` VarRef reinstated" note) — `f11_static_ref.anvl` has since been updated in place to `derived_var := $base_var;`, and `AML02` to match, closing the staleness this bullet used to flag as an open to-do.
  ...
  - `f13_include.anvl` → rewritten to valid `include "f01_bare_literal.anvl";` (double-quoted, semicolon-terminated) plus a static reference into the included document's flat namespace (`alias := name;`), instead of a single-quoted unterminated include combined with a var-ref.
  - `f35_full.anvl` is still stale (space-separated `: =`, unterminated single-quoted include) and intentionally untouched this pass — it's a larger kitchen-sink fixture and wasn't in scope this round; still tracked as an open question.
...
## Relationship to header scan and include loading
...
mod_load_includes(ctx, doc, err)
doc_parse_body(doc, err)
```

If `doc_scan_header` or `mod_load_includes` reports an error, body parsing is skipped for that document.
...
7. **Dialect gating**: enforced inline inside `doc_parse_body` via `Source.dialect(doc->source)`, failing fast through `Source.set_error` — the same pattern the header scanner already uses for AMP include/attribute rejection.
...
13. **Body-parse processing order and topological sort are both moot** — see § *Relationship to header scan and include loading* above and `notes/resolution-phase.md` for why.
...
5. **Fixture staleness** — **resolved for `f11_static_ref.anvl`**: now that `$identifier` VarRef is implemented (see **Value structure**'s "`$` VarRef reinstated" note), `f11_static_ref.anvl` was updated in place to `derived_var := $base_var;` and `AML02` to assert `VARREF`/`.varref.target` — no separate `f11_varref.anvl` file needed, since `f11_static_ref.anvl` already occupies that fixture's rightful slot. `f12_interpolated.anvl`/`f13_include.anvl`'s old `$"..."` interpolation and `$var`-inside-include syntax still predate current grammar and remain fair to retire/repurpose as AnvilScript fixtures, unchanged from the original call below.
    - `f35_full.anvl`: let's remove interpolation
    - `f12_interpolated.anvl`: definitely repurpose for **AnvilScript**
    - `f13_include.anvl`: expand concern here
