...
- **Resolved — `tuple` is AMP-legal.** Supersedes the "still undecided" note this line originally carried. Decided (unanimous, cross-project with `anvil.js`, which already ships this): a scalar-only tuple is a real, practical AMP use case, same footing as a scalar array. `AMP17` can now be written as a positive case (tuple-of-scalars accepted), not a negative one — see **AMP suite** below. Not yet implemented in `parser.c` (tuples aren't wired up at all yet); this is a grammar decision recorded ahead of that work.
...
**TODO — generalize the decline-and-rewind boundary check.** The second-decimal-point rewind above is currently the *only* case that declines instead of committing. It needs to be the general rule, not a special case: once `parse_numeric_literal` finishes consuming its digit/decimal/exponent run, check what character comes next. If it's a genuine value boundary (whitespace, `;`, `,`, `]`, `)`, `}`, EOF), the numeric literal is done and can commit. If it's anything else — most concretely a `-` immediately after a plain digit run, e.g. `09-02-2026` — the whole span was never a number and must decline (rewind to the cached entry position) so **Bare literal grammar** below gets the untouched, full token to claim instead. Today, `09-02-2026` incorrectly *succeeds* as a two-digit `NUMERIC` (`09`) and leaves `-02-2026` dangling to trip an unrelated "expected `;`" error — found while working out bare-literal coverage for `AMP03b`, not yet fixed.
...
**Mid-token characters** — permissive by default. Confirmed fine, no structural conflict, and implemented via `Source.is_bare_literal_part` (`src/core/source.c`): alpha, digit, `_`, `-`, `.`, `/`, `:`, and **`$`** (explicit decision: kept alive as an ordinary mid-token character — it only triggers VarRef dispatch as a value's *leading* character; once already inside a committed bare-literal scan, a later `$` is never re-interpreted, so `a$b` is simply the literal string `"a$b"`). `@` and `` ` `` remain undecided — both are blob-dispatch leading symbols, but by the same "only special in leading position" reasoning as `$`, there's no obvious structural reason they couldn't also be fine mid-token; not yet included in `is_bare_literal_part` pending an actual decision.
...
1. `@` and `` ` `` mid-token — allow or disallow?
...
**Status: AMP suite complete and GREEN.** `test/unit/test_body_amp.c` (28 cases, `AMP00a`–`AMP23`, `AMP17` deliberately absent — reserved) builds clean, runs clean, all 28 passing (194/194 assertions), Valgrind-clean (0 errors, 0 leaks — confirmed after the `.collection.items`/`mod_ctx_dispose` fix, see **Arena-backed allocation** — not before). The `#if ENABLED`/`#endif` scaffolding that gated cases in while `doc_parse_body` was still a stub has been removed entirely now that nothing needs gating — every case in the file compiles and runs unconditionally. `test/unit/test_body_aml.c` (12 cases, `AML00`–`AML11`) is still RED against the same real parser (3/12 passing — the 9 failures are AML-only constructs: objects, inheritance, static references, none of which are implemented yet) — this is the deliberate, expected next-up work, not a regression.
...
### AMP suite — `test/unit/test_body_amp.c`, cases AMP00a–AMP23 (`AMP17` reserved)
...
- **AMP17 — reserved, deliberately not added.** Belongs to tuple-of-scalars-in-AMP (resolved AMP-legal — see **Dialect scope** above) as a *positive* case once `parse_tuple` is implemented (still a stub — `parse_array`'s sibling, same shape, not yet built).
...
- **AML00** — tuple assignment `name := (a, b, c);` (`f06_tuple.anvl`) — since scalar tuples are now confirmed AMP-legal too (see AMP17 above), this case now only needs to cover what's AML-specific: a tuple containing non-scalar elements (object/array/nested tuple), which AMP's own scalar-only-tuple rule forbids.
