 ...
 *
 * All heap allocation goes through Allocator.alloc (sigma.memory bump allocator).
 * String building uses StringBuilder (sigma.text).
 *
 * No explicit free() calls — bump allocator resets on controller dispose.
 *
...
static bool node_add_child(asl_node_t *parent, asl_node_t *child) {
   if (parent->child_count == 0) {
   ...
   } else if ((parent->child_count & (parent->child_count - 1)) == 0 && parent->child_count >= ASL_INIT_CHILDREN) {
      /* double capacity */
      ...
      /* Old children array remains in bump allocator — no free needed */
      parent->children = nb;
...
static void node_free_impl(asl_node_t *n) {
   /* Bump allocator — no individual frees needed, nodes stay live until controller reset */
   (void)n;
}
...
static void value_free_impl(asl_value_t *v) {
   /* Bump allocator — no individual frees needed, values stay live until controller reset */
   (void)v;
}
...
static asl_node_t *parse_unary(asl_scanner_t *s, const char *src) {
   ...
   if (c == '-') {
      ...
      if (!n) {
         node_free_impl(operand);
         return NULL;
      }
      n->op = ASL_OP_NEG;
      if (!node_add_child(n, operand)) {
         node_free_impl(n);
         return NULL;
      }
      return n;
...
static asl_node_t *parse_primary(asl_scanner_t *s, const char *src) {
   ...
   if (is_digit(c) || (c == '-' && s->pos + 1 < s->end && is_digit(s->src[s->pos + 1]))) {
      ...
      if (c == '-')
         s->pos++;
      while (s->pos < s->end && is_digit(s->src[s->pos]))
         s->pos++;
      bool is_float = false;
      if (s->pos < s->end && s->src[s->pos] == '.') {
         is_float = true;
      ...
      if (is_float) {
         ...
         if (n)
            n->float_value = strtod(tmp, NULL);
      } else {
         n = node_alloc(ASL_NODE_INT_LITERAL);
         if (n)
            n->int_value = (int64_t)strtoll(tmp, NULL, 10);
      }
      /* tmp stays in bump allocator — no free needed */
      return n;
   }
...
static asl_node_t *parse_block(asl_scanner_t *s, const char *src) {
   ...
   while (true) {
      ...
      if (!stmt) {
         node_free_impl(block);
         return NULL;
      }
      if (!node_add_child(block, stmt)) {
         node_free_impl(block);
         return NULL;
      }
   }
...
static asl_node_t *parse_stmt(asl_scanner_t *s, const char *src) {
   ...
   /* return */
   if (s->pos + 6 <= s->end && strncmp(src + s->pos, "return", 6) == 0 && !is_alnum(src[s->pos + 6])) {
      s->pos += 6;
   ...
   /* break */
   if (s->pos + 5 <= s->end && strncmp(src + s->pos, "break", 5) == 0 && !is_alnum(src[s->pos + 5])) {
      s->pos += 5;
      scan_match(s, ';');
   ...
   /* continue */
   if (s->pos + 8 <= s->end && strncmp(src + s->pos, "continue", 8) == 0 && !is_alnum(src[s->pos + 8])) {
      s->pos += 8;
      scan_match(s, ';');
   ...
   /* if / else-if / else */
   if (s->pos + 2 <= s->end && strncmp(src + s->pos, "if", 2) == 0 && !is_alnum(src[s->pos + 2])) {
      s->pos += 2;
      scan_skip_ws(s);
      if (s->pos >= s->end || s->src[s->pos] != '(') {
      ...
      if (!if_node) {
         node_free_impl(cond);
         node_free_impl(then_block);
         return NULL;
      }
      if (!node_add_child(if_node, cond) || !node_add_child(if_node, then_block)) {
         node_free_impl(if_node);
         return NULL;
      }

      /* optional else / else-if */
      scan_skip_ws(s);
      if (s->pos + 4 <= s->end && strncmp(src + s->pos, "else", 4) == 0 && !is_alnum(src[s->pos + 4])) {
         s->pos += 4;
         scan_skip_ws(s);
   ...
   if (s->pos + 3 <= s->end && strncmp(src + s->pos, "for", 3) == 0 && !is_alnum(src[s->pos + 3])) {
      s->pos += 3;
      scan_skip_ws(s);
      if (s->pos >= s->end || s->src[s->pos] != '(') {
         anvl_error_set(ANVL_ERR_ASL_PARSE_ERROR,
      ...
      if (s->pos < s->end && s->src[s->pos] != ')') {
         ...
         if (s->pos + 1 < s->end && s->src[s->pos] == '+' && s->src[s->pos + 1] == '+') {
            /* i++ → assign(i, i+1) */
            s->pos += 2;
            ...
            add->op = ASL_OP_ADD;
            node_add_child(add, ref);
            node_add_child(add, one);
         ...
         } else if (s->pos + 1 < s->end && s->src[s->pos] == '-' && s->src[s->pos + 1] == '-') {
            /* i-- → assign(i, i-1) */
            s->pos += 2;
            asl_node_t *one = node_alloc(ASL_NODE_INT_LITERAL);
            one->int_value = 1;
            ...
            sub->op = ASL_OP_SUB;
            node_add_child(sub, ref);
            node_add_child(sub, one);
            step_node = node_alloc(ASL_NODE_ASSIGN);
            node_add_child(step_node, lhs_id);
            ...
            if (aug != ASL_OP_NONE) {
               ...
               binop->op = aug;
               node_add_child(binop, ref2);
               node_add_child(binop, rhs);
               rhs = binop;
            }
            step_node = node_alloc(ASL_NODE_ASSIGN);
            node_add_child(step_node, lhs_id);
      ...
      if (!node_add_child(for_node, init_node)) {
         node_free_impl(for_node);
         return NULL;
      }
      if (!node_add_child(for_node, cond_node)) {
         node_free_impl(for_node);
         return NULL;
      }
      if (!node_add_child(for_node, step_node)) {
         node_free_impl(for_node);
         return NULL;
      }
      if (!node_add_child(for_node, body)) {
         node_free_impl(for_node);
         return NULL;
      }
      return for_node;
    ...
    * Peek ahead: if we see  ident ('='|'+='|'-='|'*='|'/=')  → assign */
   {
      ...
      if (is_alpha(src[s->pos])) {
         ...
         if (s->pos + 1 < s->end && s->src[s->pos + 1] == '=' && (s->src[s->pos] == '+' || s->src[s->pos] == '-' || s->src[s->pos] == '*' || s->src[s->pos] == '/')) {
            static const char ops[] = "+-*/";
            static const asl_op_t op_map[] = {ASL_OP_ADD, ASL_OP_SUB, ASL_OP_MUL, ASL_OP_DIV};
            for (int qi = 0; qi < 4; qi++) {
               if (s->src[s->pos] == ops[qi]) {
                  aug = op_map[qi];
                  break;
               }
            }
            s->pos += 2;
            is_assign = true;
         } else if (s->pos < s->end && s->src[s->pos] == '=' && (s->pos + 1 >= s->end || s->src[s->pos + 1] != '=')) {
            s->pos++;
            is_assign = true;
         } else if (s->pos + 1 < s->end && s->src[s->pos] == '+' && s->src[s->pos + 1] == '+') {
            aug = ASL_OP_ADD;
         ...
         if (is_assign) {
            ...
            if (aug == ASL_OP_ADD || aug == ASL_OP_SUB) {
               ...
               if (s->pos >= s->end || s->src[s->pos] == ';' || s->src[s->pos] == '}') {
                  ...
                  asl_node_t *bin = node_alloc(ASL_NODE_BINARY_EXPR);
                  bin->op = aug;
                  node_add_child(bin, ref);
                  node_add_child(bin, raw);
                  rhs = bin;
               }
...
static bool scope_set(asl_scope_impl_t *sc, const char *name, int namelen,
   ...
   for (int i = 0; i < sc->count; i++) {
      if (strncmp(sc->names[i], name, (usize)namelen) == 0 && sc->names[i][namelen] == '\0') {
         value_free_impl(&sc->values[i]);
         return value_copy(&sc->values[i], val);
      }
   }
   /* Insert new */
   if (sc->count >= ASL_MAX_SCOPE)
      return false;
   int idx = sc->count++;
...
static bool eval_expr(asl_eval_ctx_t *ctx, const asl_node_t *node, asl_value_t *out) {
   ...
   case ASL_NODE_VAR_REF: {
      ...
      if (ctx->ext_lookup) {
         ...
         /* key stays in bump allocator — no free needed */
         if (found)
            return true;
      }
      char msg[96];
   ...
   case ASL_NODE_UNARY_EXPR: {
      ...
      if (!eval_expr(ctx, node->children[0], &operand))
         return false;
   ...
   case ASL_NODE_BINARY_EXPR: {
      ...
      if (node->op == ASL_OP_AND) {
         asl_value_t lv = {0};
         if (!eval_expr(ctx, node->children[0], &lv))
            return false;
         if (!asl_truthy(&lv)) {
            value_free_impl(&lv);
            out->kind = ASL_BOOL;
      ...
      /* String concat */
      if (node->op == ASL_OP_ADD && (lv.kind == ASL_STRING || rv.kind == ASL_STRING)) {
         ...
         if (lv.string_value)
            memcpy(buf, lv.string_value, llen);
         if (rv.string_value)
            memcpy(buf + llen, rv.string_value, rlen);
         buf[llen + rlen] = '\0';
         value_free_impl(&lv);
         value_free_impl(&rv);
         out->kind = ASL_STRING;
         out->string_value = buf;
         return true;
      }
      ...
      case ASL_OP_ADD:
         if (use_float) {
            out->kind = ASL_FLOAT;
            out->double_value = ld + rd;
         } else {
            out->kind = ASL_INT;
            out->long_value = li + ri;
         }
         return true;
      case ASL_OP_SUB:
         if (use_float) {
      ...
      case ASL_OP_EQ:
         out->kind = ASL_BOOL;
         if (use_float)
            out->bool_value = (ld == rd) ? 1 : 0;
         else
            out->bool_value = (li == ri) ? 1 : 0;
         return true;
      case ASL_OP_NOT_EQ:
         out->kind = ASL_BOOL;
         if (use_float)
   ...
   case ASL_NODE_CALL: {
      ...
      if (argc > 0) {
         ...
         for (int i = 0; i < argc; i++) {
            if (!eval_expr(ctx, node->children[i + 1], &args[i])) {
               for (int j = 0; j < i; j++)
                  value_free_impl(&args[j]);
               /* args stay in bump allocator — no free needed */
               return false;
            }
         }
      }
      ...
      if (callee->kind == ASL_NODE_MEMBER_ACCESS) {
         ...
         for (int mi = 0; mi < ctx->module_count; mi++) {
            if (strncmp(ctx->modules[mi].name, module_name, (usize)module_len) == 0 && ctx->modules[mi].name[module_len] == '\0') {
               ...
               *out = ctx->modules[mi].call(fn_buf, args, argc,
               ...
               /* fn_buf stays in bump allocator — no free needed */
               ok = true;
               break;
            }
         }
   ...
   case ASL_NODE_MEMBER_ACCESS: {
      ...
      asl_value_t obj = {0};
      if (!eval_expr(ctx, node->children[0], &obj))
         return false;
      if (obj.kind == ASL_MAP) {
         const char *mname = ctx->src + node->member_start;
         int mlen = node->member_length;
         for (int i = 0; i < obj.map_count; i++) {
            if (strncmp(obj.map_keys[i], mname, (usize)mlen) == 0 && obj.map_keys[i][mlen] == '\0') {
...
static bool eval_stmt(asl_eval_ctx_t *ctx, const asl_node_t *node) {
   ...
   case ASL_NODE_RETURN: {
      asl_value_t val = {0};
      if (node->child_count > 0) {
         if (!eval_expr(ctx, node->children[0], &val))
            return false;
      }
   ...
   case ASL_NODE_ASSIGN: {
      asl_value_t val = {0};
      if (!eval_expr(ctx, node->children[1], &val))
         return false;
   ...
   case ASL_NODE_IF: {
      ...
      if (!eval_expr(ctx, node->children[0], &cond_val))
         return false;
   ...
   case ASL_NODE_FOR: {
      ...
      /* init */
      if (init_n && !eval_stmt(ctx, init_n))
         return false;
      ...
      while (true) {
         ...
         /* step */
         if (step_n && !eval_stmt(ctx, step_n))
            return false;
      }
      return true;
   }
...
static bool eval_block(asl_eval_ctx_t *ctx, const asl_node_t *block) {
   for (int i = 0; i < block->child_count; i++) {
      if (!eval_stmt(ctx, block->children[i]))
         return false;
      if (ctx->has_return || ctx->has_break || ctx->has_continue)
         break;
   }
   return true;
}
