...
**`Anvil` group — landed.** `anvil_load(filepath) -> anvil_document` runs the full pipeline in one call (`mod_ctx_initialize` → `doc_load_source` → `mod_ctx_register_doc` → `doc_scan_header` → `mod_load_includes` → `mod_ctx_create_arena` → `doc_parse_body` for every document in the include graph → `mod_resolve_context`), stopping at the first failing phase. Rather than translating whatever internal `anvl_err_code` a failing call happened to produce, `anvil_load` just records *which phase* it was in when something failed (`ANVIL_ERR_IO`/`HEADER`/`INCLUDE`/`SYNTAX`/`RESOLVE`/`MEMORY`) directly on the handle — simpler than a translation table and immune to internal code churn by construction, not just by convention. A nested include's own header-scan failure is deliberately categorized as `ANVIL_ERR_INCLUDE`, not `ANVIL_ERR_HEADER` — `mod_load_includes` scans each child's header as it recurses, and the *root* document's own header was fine, so "something went wrong resolving the include graph" is the more accurate framing from the caller's perspective.
...
New fixtures: `anvil_err_header_dup_shebang.anvl` (duplicate `#!aml`), `anvil_err_resolve_duplicate.anvl` (includes `resolver_dup_include.anvl` and redeclares the same name) — reusing existing fixtures (`f01_bare_literal.anvl`, `body_err_unterminated_block.anvl`, `hdr_include_missing.anvl`) for the other categories.

**`Statement`/`Value` groups — landed.** `anvil_document_find_statement(doc, name)` (originally `anvil_statement_get`, renamed — see below) looks up a top-level statement directly against `ctx->identifiers` (the resolver's own map — no separate lookup structure needed). `anvil_statement`/`anvil_value` are implemented as plain pointer casts of their internal counterparts (`anvl_statement`/`anvl_value`), not separately-allocated wrapper structs like `anvil_document` needed — safe because the caller can never dereference an opaque handle directly, and lifetime is already governed by the same arena/one-disposal-pass guarantee `.varref.resolved`'s pointer-aliasing relies on. `anvil_value_get_statement` reuses the exact same `Statement` accessors for walking an `OBJECT`'s nested fields as for top-level lookup — one consistent abstraction, not a separate name-based field accessor. `anvil_statement_get_name` exists specifically to make that nested case usable (a top-level caller already knows the name they looked up; an object-field walker doesn't).
...
**Document include iterator — landed, prerequisite for `notes/native-schema.md`'s types work.**
(Originally shipped as `anvil_document_get_imports`/AML's `import` keyword; renamed to
`anvil_document_get_includes`/`include` — see
[`FR-2609-anvl-public-api-002`](FR-2609-anvl-public-api-002.md).)
`anvil_document_get_includes(doc) -> anvil_document_iterator`, `anvil_document_iterator_next`,
`anvil_document_iterator_dispose` — a heapless HPS scan (`List.as_queryable` over
`doc->root->header->includes`) over a document's own *direct* includes only, mirroring
`anvil_statement_iterator`'s shape exactly, added because the public flat API previously only
ever exposed the root document — `include "file.types.anvl";` already fully loads and resolves
transitively today, but nothing let a caller reach an included document's own statements/
...
one synthesized by the include iterator) makes `anvil_dispose` safe either way — true does the
...
**`Document.find_statement` / `Value.find_statement` — landed, prerequisite for the C# binding.**
Surfaced designing `anvil.net`'s real binding (lazy, OOP-native navigation via the vtable directly
— see that repo's own design record) — two small, self-contained fixes, zero ripple into
`anvil-node`/`anvil-wasm` (checked directly: neither calls `anvil_statement_get`, only the
index-based `anvil_value_get_statement`).

1. **Renamed `anvil_statement_get(doc, name)` → `anvil_document_find_statement(doc, name)`**,
   regrouped from the `Statement` vtable to `Document`. The flat API already had an established
   convention — `find_*` for by-key lookups that can fail (`anvil_document_find_attribute`,
   `anvil_statement_find_attribute`), `get_*` for positional/unconditional access.
   `anvil_statement_get` broke that convention (a by-name lookup named "get") and was grouped
   under the wrong vtable (its receiver is a `Document`, not a `Statement`). Same behavior, name
   and grouping only.
2. **Added `anvil_value_find_statement(val, key)`** — the missing sibling to the existing
   `anvil_value_get_statement(val, index)`, mirroring the `get_attribute`/`find_attribute` pair
   that already coexists for attributes. Until now there was no native by-key lookup into a
   nested object's fields at all — every binding wanting one had to build its own workaround. A
   real, reusable primitive for any binding doing lazy/vtable-style navigation instead of
   eager-JSON (`anvil-node`/`anvil-wasm`'s own strategy, which never needed this since they always
   eagerly enumerate every field via the index accessor).

`ANV37` (`test_anvil_native.c`) covers `anvil_value_find_statement` directly (found, found-not-first,
absent key, NULL safety, non-OBJECT value); `VT03`/`VT05` (`test_anvil_vtable.c`) updated for the
new/moved vtable fields. Full 14-suite regression green, Valgrind-clean, `test/functional` green
across all four build variants (static/shared × debug/release).
...
where that collided with Anvil Native's own include-registry dedup. That reasoning has nothing to
